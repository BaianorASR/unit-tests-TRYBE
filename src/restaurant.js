...
const createMenu = (myMenu) => ({
  ...
  order,
  pay: () => {
   const DRINK = objectCount(restaurant.fetchMenu().drink); 
   const FOOD = objectCount(restaurant.fetchMenu().food);  
   return ((10 / 100) + 1) * (DRINK + FOOD);
  },
});

restaurant = createMenu({ food: { coxinha: 3.9, sopa: 9.9 }, drink: { agua: 3.9, cerveja: 6.9 } });
